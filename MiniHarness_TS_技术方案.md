...
| `evaluation/` | 评估与质量保障 | 基于 EngineEvent 的步骤级、轨迹级、任务级评估，聚合指标和基线回归检查 | 第八阶段 |
...
flowchart TD
    ...
    C --> L[evaluation Offline Metrics / Regression Gates]
```
...
quality.averageRunDurationMs
quality.averageTokensPerRun
quality.errorRecoveryRate
quality.duplicateToolCallRate
```
...
quality.taskSuccessRate / averageRunDurationMs / averageTokensPerRun
quality.errorRecoveryRate / duplicateToolCallRate
...
其中 `quality` 字段用于生产进程内的连续质量概览：任务成功率来自 completed/start 比例，平均运行耗时来自 `agent_start` 到 `agent_end`，平均 token 来自 `model_message.usage`，错误恢复率和重复调用率来自连续 `tool_result` 事件。它对应生产指南中的结构化可观测性、健康检查和持续评估输入，但暂不绑定外部 metrics backend。

## 5.11 `evaluation/` 评估与质量保障

`evaluation/` 承载第十三章评估与质量保障的轻量落地能力。它不调用模型、不执行工具，只消费 `EngineEvent[]`，因此既能用于 mock E2E 测试，也能用于真实运行轨迹、benchmark 子集和发布前质量门禁。
...
| 层级 | 指标 | 来源 |
|---|---|---|
| 步骤级 | `toolAccuracy`、`parameterAccuracy`、`executionSuccessRate` | `model_message.toolCalls`、`tool_result.success` |
| 轨迹级 | `trajectoryEfficiency`、`duplicateRate`、`errorRecoveryRate` | 按时间排序的 `tool_result` |
| 任务级 | `successRate`、`durationMs`、`totalTokens`、`costUsd` | `agent_start`、`agent_end`、`runtime_error`、`model_message.usage` |

综合评分采用保守权重：任务成功 40%、轨迹效率 30%、步骤准确 20%、成本效率 10%。额外工具调用主要通过轨迹效率和重复调用率扣分，避免把“失败后恢复”和“第一步工具选择错误”混在同一个指标里。

### 5.11.2 聚合与基线回归

测试集或 benchmark 运行后使用：

```ts
const aggregate = aggregateEvaluationResults(results);
const regressions = checkEvaluationRegression(current, baseline, {
...
- 越高越好：任务成功率、执行成功率、轨迹效率、综合评分。
- 越低越好：平均耗时、平均成本、重复工具调用率。

返回的回归项包含 metric、baseline、current、changeRatio 和 direction，可直接用于 CI 输出、发布门禁或人工评审清单。

### 5.11.3 与生产指标的边界

```text
evaluation/index.ts           离线评估、E2E、benchmark、发布前质量门禁
production/metrics.ts quality 进程内实时质量概览、健康告警输入
orchestration/evaluator.ts    PGE 独立 evaluator 文本输出解析
```

三者共享运行事件和结果语义，但职责不同：`evaluation/` 做可复现评分，`production/metrics.ts` 做连续聚合，`orchestration/evaluator.ts` 做独立评审结果解析。
...
目标：基于第十三章评估指南，建立步骤级、轨迹级、任务级的离线评估能力，并把生产指标扩展到可持续观察质量信号。
...
production/metrics.ts
src/index.ts
docs/评估与质量保障优化技术实现.md
MiniHarness_TS_技术方案.md
```

验收标准：

```text
1. evaluateEngineRun() 可以从 EngineEvent[] 计算步骤、轨迹、任务和成本指标。
2. aggregateEvaluationResults() 可以汇总多条运行结果。
3. checkEvaluationRegression() 可以识别成功率下降、耗时上升和综合评分下降。
4. ProductionMetricsCollector.snapshot().quality 输出任务成功率、平均运行耗时、平均 token、错误恢复率和重复工具调用率。
5. evaluation 能从 src/index.ts 公共入口导出。
6. 新增技术实现文档说明评估方案、测试策略和后续 benchmark 扩展边界。
```
...
14. EngineEvent 轨迹可以被 evaluation 模块转换为离线质量指标。
```
...
| tests/ | 单元测试和集成测试 |
| 架构文档 | 本技术方案、模块设计文档、生产级优化技术实现文档、Harness 安全体系优化技术实现文档、评估与质量保障优化技术实现文档 |
...
evaluation 做离线质量评估、聚合指标和基线回归检查
reliability 保证稳定性
security 保证安全边界
utils 提供基础辅助能力
```
...
这条链路跑通后，再逐步扩展工具执行、安全控制、真实模型、MCP 集成、任务编排、生产监控、质量评估和更完整的控制平面。

生产化和评估阶段的目标不是一次性引入完整平台，而是在不破坏现有接口的前提下，让 Harness 拥有可灰度、可观测、可度量、可回归检查和可继续扩展的横切基础。
