...
| `production/` | 生产化横切层 | Feature Gates、模块化提示词、Schema cache、运行指标汇总 | 第六阶段 |
...
flowchart TD
    ...
    C --> K[production Feature Gates / Prompt / Metrics]
```
...
runtime.startedRuns / completedRuns / errorCount / turnCount
```

这对应生产指南中的结构化可观测性，但暂不绑定外部 metrics backend。
